...
def synthesize(args, model, _stft):   
    ...
    torch.save(mel_output.detach().cpu().float().numpy(), "./mel_output.pt")
    
    mel_ref_ = ref_mel.cpu().squeeze().transpose(0, 1).detach()
    
    mel_ = mel_output.cpu().squeeze().transpose(0, 1).detach()
    # np.save("../hifi-gan-master/test_mel_files/mel_array.npy", mel_.numpy())
    # plotting
    ...
    print('Generate done!')
    return mel_output.detach().cpu().float().numpy()
...
def get_model(checkpoint_path):
    ...
    model = get_StyleSpeech(config, checkpoint_path)
    print('model is prepared')
    return model,config

def get_mel(model,config,args):
    _stft = Audio.stft.TacotronSTFT(
                ...
                config.mel_fmin,
                config.mel_fmax)
    mel_ref_ = synthesize(args, model, _stft)
    return mel_ref_
